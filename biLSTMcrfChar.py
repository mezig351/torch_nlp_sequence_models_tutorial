...
class BiLSTM_CRF(nn.Module):
    ...
    def __init__(self, vocab_size, tag_to_ix, embedding_dim, hidden_dim, char_embedding_dim, charset_size):
    ...
    def _get_lstm_features(self, sentence, char_sentence, char_lengths, perm_idx):
        ...
        self.hidden = self.init_hidden()
        embeds = word_char_embeds.view(len(sentence), 1, -1)
        lstm_out, self.hidden = self.lstm_word(embeds, self.hidden)
    ...
    def _viterbi_decode(self, feats):
        ...
        # path_score = terminal_var[0][best_tag_id]
            ...
            bptrs_t = bptrs_t.squeeze().data.numpy()  #cpu
            next_tag_var = next_tag_var.data.numpy()
    ...
    # def viterbi_loss(self, sentence, char_sentence, char_lengths, perm_idx, tags):
    #     feats = self._get_lstm_features(sentence, char_sentence, char_lengths, perm_idx)
    #     viterbi_score, viterbi_tags = self._viterbi_decode(feats)
    #     if viterbi_tags != tags.tolist():
    #         gold_score = self._score_sentence(feats, tags)
    #         return (viterbi_score - gold_score), viterbi_tags
    #     else:
    #         return torch.zeros(1), viterbi_tags
    ...
    def forward(self, sentence, char_sentence, char_lengths, perm_idx):  # dont confuse this with _forward_alg above.
        # Get the emission scores from the BiLSTM
        lstm_feats = self._get_lstm_features(sentence, char_sentence, char_lengths, perm_idx)
...
model = BiLSTM_CRF(len(word_to_ix), tag_to_ix, EMBEDDING_DIM, HIDDEN_DIM, CHAR_EMBEDDING_DIM, len(char_to_ix))
...
with torch.no_grad():
    ...
    print(model(precheck_sent, char_inputs, char_lengths, perm_idx))
...
for epoch in range(
    ...
    for sentence, tags in training_data:
        ...
        loss = model.neg_log_likelihood(sentence_in, char_sentence_in, char_lengths, perm_idx, targets)
        # print(loss)
...
# (tensor(20.4906), [0, 1, 1, 1, 2, 2, 2, 0, 1, 2, 2])  [0, 1, 1, 1, 1, 2, 2, 0, 1, 2, 2])

t1 = datetime.datetime.now()

print('Total Time:', t1-t0) #0:00:07.960584